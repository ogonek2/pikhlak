...
class OpenAiProvider implements AiProviderInterface
{
    public function chat(array $messages, float $temperature, int $maxTokens, ?string $modelName = null): array
    {
        ...
        $model = $modelName ?? AiModelNameResolver::resolve(null, 'openai');
        $start = microtime(true);

        $http = Http::withToken($key)->timeout(90);
        if (! config('ai.verify_ssl')) {
            $http = $http->withOptions(['verify' => false]);
        }

        $response = $http->post(config('ai.providers.openai.base_url').'/chat/completions', [
            'model' => $model,
            'messages' => $messages,
